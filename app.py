...
# --- タブの作成 ---
tab1, tab2 = st.tabs(["📧 メール作成 (AI提案)", "➕ 案件登録 (テキスト解析)"])

# ==========================================
# タブ1：いつものメール作成機能
# ==========================================
with tab1:
    st.markdown("### 📝 候補者へのメールを作成")
    notes = st.text_area("面談メモ", height=150, placeholder="中島さん、30代、Webデザイン経験あり...", key="note_input")

    if st.button("🚀 メールを作成する", type="primary"):
        if not notes:
        ...
        以下の【面談メモ】と【案件リスト】をもとに、候補者に最適な案件を1つだけ提案してください。
        ...
        2. メール本文は、以下の【出力フォーマット】を一言一句変えずに使うこと。
        3. 案件情報の部分は、提供された【案件記載スタイル】（■を使う形式）を完全に再現すること。
        ...
        try:
            ...
            status_area.success("✨ メール完成！")
            st.text_area("生成結果", value=response.text, height=600)
